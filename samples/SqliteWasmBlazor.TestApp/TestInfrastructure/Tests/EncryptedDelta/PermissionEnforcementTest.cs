...
internal class PermissionEnforcementTest(
    ...
    Crypto.Abstractions.ICryptoProvider cryptoProvider,
    ...
    : CryptoSyncTestBase(cryptoFactory, databaseService, session)
{
    ...
    private async ValueTask SetSenderRoleAsync(SyncRole role)
    {
        ...
        // Clone the static AdminSeed byte[] so ZeroMemory below doesn't
        // clobber the AppDomain-shared seed for subsequent test steps.
        var adminEd25519Priv = CryptoTestContext.AdminEd25519PrivateKey.AsSpan().ToArray();
