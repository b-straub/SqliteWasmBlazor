...
public class CrossLanguageEciesVectorTests
{
    ...
    public async Task EciesCombinator_ProductionDecryptRoundTrips(
        ...
        string expectedCiphertextBase64)
    {
        // The production `DecryptAsymmetricAsync` consumes the same wire shape
        // (EphemeralPublicKey, Ciphertext, Nonce). Confirm round-trip with the
        // golden bytes against the recipient's actual private key.
        var recipient = await _crypto.DeriveDualKeyPairAsync(recipientSeed);
        var recipientPriv = Convert.FromBase64String(recipient.X25519PrivateKey);
        ...
        var result = await _crypto.DecryptAsymmetricAsync(encrypted, recipientPriv);

        Assert.True(result.Success, $"DecryptAsymmetricAsync failed: {result.ErrorCode}");
        ...
        byte[] ephSeed, byte[] recipientSeed, byte[] nonce, string plaintext)
    {
        ...
        var ephPriv = Convert.FromBase64String(ephemeral.X25519PrivateKey);
