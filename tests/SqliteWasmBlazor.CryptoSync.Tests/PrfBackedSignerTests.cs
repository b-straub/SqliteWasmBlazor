...
public class PrfBackedSignerTests
{
    ...
    // PrfBackedSenderAuthSigner / PrfBackedReceiveAuthSigner route through.
    ...
    private static (PrfBackedSenderAuthSigner SenderSigner,
                    ...
                    StubPrfService PublicKeyProvider)
        BuildSigners()
    {
        var provider = new FakeKeyIdCryptoProvider();
        var prfStub = new StubPrfService();
        var prfOptions = Options.Create(new PrfOptions { Salt = Salt });
        var sender = new PrfBackedSenderAuthSigner(prfStub, provider, prfOptions);
        var receive = new PrfBackedReceiveAuthSigner(prfStub, provider, prfOptions);
        return (sender, receive, provider, prfStub);
    }

    private static async Task<string> SeedSessionAsync(
        FakeKeyIdCryptoProvider provider,
        StubPrfService prfStub,
        byte[] seed)
    {
        ...
        prfStub.Ed25519PublicKey = stored.Value!.Ed25519PublicKey;
