...
public class CrossLanguageKdfVectorTests
{
    ...
    public async Task PrintGoldenVectors()
    {
        foreach (var entry in Vectors)
        {
            ...
            _output.WriteLine($"  x25519Priv = {Convert.ToBase64String(dual.X25519PrivateKey)}");
            _output.WriteLine($"  x25519Pub  = {dual.X25519PublicKey}");
            _output.WriteLine($"  ed25519Priv = {Convert.ToBase64String(dual.Ed25519PrivateKey)}");
        ...
        string expectedEd25519Pub)
    {
        ...
        Assert.Equal(expectedX25519Priv, Convert.ToBase64String(dual.X25519PrivateKey));
        Assert.Equal(expectedX25519Pub, dual.X25519PublicKey);
        Assert.Equal(expectedEd25519Priv, Convert.ToBase64String(dual.Ed25519PrivateKey));
