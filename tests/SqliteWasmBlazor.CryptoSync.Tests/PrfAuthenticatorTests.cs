...
public class PrfAuthenticatorTests
{
    ...
    public async Task RegisterAsync_RunsCreateThenDerive_ReturnsBothPieces()
    {
        ...
        Assert.Single(stub.DeriveKeysCredentialIds, CredentialId); // RawId, not Id; Register does not persist the hint.
    }
    ...
    public async Task AuthenticateAsync_WithHint_RoutesThroughDeriveKeysAsync()
    {
        ...
        Assert.Equal(0, stub.DeriveKeysDiscoverableCalls);
    }

    [Fact]
    public async Task AuthenticateAsync_NoHint_RoutesThroughDeriveKeysDiscoverableAsync()
    {
        var stub = new StubPrfService
        {
            DeriveKeysDiscoverable = () => ValueTask.FromResult(
        ...
        Assert.Equal(1, stub.DeriveKeysDiscoverableCalls);
        Assert.Empty(stub.DeriveKeysCredentialIds);
    }
