...
| 2 | **Test buildup, three rounds.** R1 (deterministic) ✅ DONE — R1.1 KDF byte-equality (`5fc8a62`); R1.2 canonical envelope agreement + protocol fix `7d77f00` so C# hashes raw ciphertext bytes (matching TS noble); R1.3 `SigningService` keyId-cache; R1.4 `AsymmetricEncryptionService` keyId-cache; R1.5 JS bridge dispatch (storeKeys / signWithCachedKey / encrypt+decrypt round-trips, ttl-null path, AAD binding). All R1.2-R1.5 land in `69c2b41`. **Pre-R2 interop sweep (planned, fresh-context):** before starting R2, run a 30-60min scan for sibling C#↔TS divergences in remaining cross-language primitives (`WrapContentKey`/`UnwrapContentKey`, `DeriveWrappingKey`, ECIES envelope serialization, push payload framing). Pattern: fixed-input vector + collapsed golden, mirrored xUnit ↔ vitest, same shape as R1.2. Cheap insurance — if R1.2 found one, siblings might exist; if the sweep is clean, R2 starts on solid ground. R2 (virtual authenticator E2E): `WaFixtureBase` virtual-authenticator setup + four PRF scenarios (register, unlock+open, mismatch, rekey). R3 (composition): synthetic-PRF-seed → VFS + delta round-trip, sync-engine `KeyRotation` → VFS rekey correctness. Bootstrap `Crypto.UI.Tests` + `CryptoSync.UI.Tests` xUnit projects (absorbs Stage 2 slot 10). | 🟡 R1 done; sweep + R2 + R3 pending |
