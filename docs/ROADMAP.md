...
_Single source of truth for "where are we." Last updated 2026-05-01 against branch `crypto-sync` HEAD `6feddb9` (Phase 1 of plane-separation + test buildup is fully shipped; **Phase 2 R1 + R2 + R3 are complete** — R2 session-expired follow-up (6/6 PRF E2E), `IPasskeyHintProvider` shipped with localStorage default + hint-aware ceremony (`ad8c7d8`), `chore(build)` stops tracking esbuild bundles + sqlite3.wasm copy (`4abc0fb`), R3.1 synthetic-seed → encrypted VFS round-trip (`0dd2516`), R3.2 synthetic-seed-driven rekey correctness (`6feddb9`). — all five deterministic-unit slots green: KDF byte-equality vectors, canonical envelope agreement, SigningService + AsymmetricEncryptionService keyId-cache round-trips, JS bridge dispatch. **Pre-R2 interop sweep is also complete** (`1a62974`, on top of the `0fa6708` bundle refresh) — three new cross-language vector files: (a) `CrossLanguageWrappingKeyVectorTests` / `crossLanguageWrappingKey.test.ts` pin `DeriveWrappingKey` (X25519 ECDH + HKDF) byte-for-byte across three contexts, (b) `CrossLanguageAesGcmVectorTests` / `crossLanguageAesGcm.test.ts` pin AES-256-GCM at fixed nonce (the underlying primitive of `WrapContentKey`/`UnwrapContentKey`) across zeros/ones/sequential keys, with-and-without AAD, (c) `CrossLanguageEciesVectorTests` / `crossLanguageEcies.test.ts` pin the full ECIES combinator (X25519 ECDH + HKDF "ecies-aes-gcm" + AES-256-GCM at fixed nonce) and verify production decrypt round-trips the captured ciphertexts. Push payload framing was scoped out — `encryptPushPayload` exists only on the TS side, no C# counterpart for cross-language equality. Plus follow-up cleanups: `8bc0af2` dropped the orphaned `crypto-core/dist/` tsc build (npm publishing is out of scope, esbuild + vitest read TS source via the package `main` field), and `2c716ca` flattened the `TypeScript-Crypto/packages/crypto-core/` workspace into a single package — primitives now live at `TypeScript-Crypto/src/crypto-core/`, tests at `TypeScript-Crypto/tests/`. The R1.2 vector exposed and resolved a real C#↔TS interop bug — `GroupEncryptionService.BuildCanonicalEnvelope` was hashing the UTF-8 bytes of the base64 ciphertext string while the TS `buildCanonicalEnvelope` hashed raw bytes; commit `7d77f00` fixes the C# side to decode-then-hash so envelopes agree byte-for-byte. The pre-R2 sweep itself was clean — every new vector landed green on first run, no further interop bugs surfaced. **Next up:** Phase 2 R2 — virtual-authenticator E2E. Pin Playwright virtual-authenticator API version, build `WaFixtureBase` with `IPage.Context.AddVirtualAuthenticatorAsync`, then four PRF scenarios (register, unlock+open, mismatch, rekey). Stage A (whitelist-broadcast rewrite) is fully complete and the Codex memory-hygiene audit close-out (zeroization + binary boundary + non-extractable JS keys + second-pass HKDF/seed/packed-buffer cleanup + third-pass B64-result-buffer clears + storeKeys failure-path scoping + legacy single-key API removal) is in. **Stage 2 (UI absorption) panels are all ✅ except slot 10 (xUnit bootstrap), which folds into Phase 2 of the active workstream — `plane-separation-and-test-buildup.md` is the active plan.** The Post-Stage-2 sequence (WebAuthn demo / TestApp / admin-seeding) sequences on top of the plane-separated structure rather than on Stage 2's monolithic UI library. Stage B (production PRF/WebAuthn signer wiring) is folded into Phase 3 of the new plan — it remains not a standalone item._
...
| 2 | **Test buildup, three rounds.** R1 (deterministic) ✅ DONE — R1.1 KDF byte-equality (`5fc8a62`); R1.2 canonical envelope agreement + protocol fix `7d77f00` so C# hashes raw ciphertext bytes (matching TS noble); R1.3 `SigningService` keyId-cache; R1.4 `AsymmetricEncryptionService` keyId-cache; R1.5 JS bridge dispatch (storeKeys / signWithCachedKey / encrypt+decrypt round-trips, ttl-null path, AAD binding). All R1.2-R1.5 land in `69c2b41`. **Pre-R2 interop sweep ✅ DONE** — three new vector files mirror xUnit ↔ vitest: (a) `CrossLanguageWrappingKeyVectorTests` / `crossLanguageWrappingKey.test.ts` pin `DeriveWrappingKey` (X25519 ECDH + HKDF) byte-for-byte across three contexts, (b) `CrossLanguageAesGcmVectorTests` / `crossLanguageAesGcm.test.ts` pin AES-256-GCM at fixed nonce (with-and-without AAD) — the underlying primitive of `WrapContentKey`/`UnwrapContentKey`; production wrappers can't be vector-tested directly because they generate a random nonce internally, so the TS side drives `crypto.subtle.encrypt` directly with the fixed nonce, (c) `CrossLanguageEciesVectorTests` / `crossLanguageEcies.test.ts` pin the full ECIES combinator (X25519 ECDH + HKDF "ecies-aes-gcm" + AES-256-GCM at fixed nonce); production `decryptAsymmetric` then round-trips the captured wire bytes back to plaintext. Push payload framing scoped out — `encryptPushPayload` is worker-only with no C# counterpart, no cross-language equality to assert. Sweep was clean; no further interop bugs surfaced. **R2 (virtual authenticator E2E) — 4 of 4 scenarios green.** New infra: `PrfWaFixture` (Kestrel on port 7060 + shared Chromium) + `PrfScenario` (per-test isolated `IBrowserContext` + CDP virtual authenticator with `hasPrf: true`, single-source `WebAuthn.addVirtualAuthenticator` shape so future Chromium drift only touches one method) + `PlaywrightInstaller` shared install gate. Tests drive `PrfVfsTest.razor` through Playwright locators. ✅ Scenario 1 — registration happy path; ✅ Scenario 2 — cached-credential round-trip (Register → AuthAndOpen → Insert+Read 25 → Read row count); ✅ Scenario 3 — credential mismatch via the page's documented multi-passkey path: register A → AuthAndOpen → Insert 25 → Lock → register B on the **same** virtual authenticator → `WebAuthn.removeCredential(A.id)` so only B answers the next discoverable PRF → AuthAndOpen → expect "Wrong passkey for this DB"; ✅ Scenario 4 — same-passkey rekey ceremony (paste own armored pubkey → Rotate → reopen → row count survives). **Lessons locked in:** Chrome only permits one `transport: "internal"` virtual authenticator at a time; mid-page `removeVirtualAuthenticator` + `addVirtualAuthenticator` swap dead-locks `navigator.credentials.create`, so multi-credential on a single authenticator + `WebAuthn.removeCredential` is the only reliable shape for mismatch tests; MudTextField `@bind-Value` defaults to OnChange (blur), so `FillAsync` must be followed by `Page.Keyboard.PressAsync("Tab")` to flush to C#; dev-friendly timeouts are 60s only for the first cold-WASM-boot click, 8-10s thereafter (per user feedback). **Production bug fix surfaced by R2:** `NobleCryptoProvider.HasCachedKey` / `RemoveCachedKey` and `VapidCryptoProvider.IsLoaded` / `ClearKey` were calling `[JSImport]` sync methods that abort the WASM runtime if the noble JS module isn't loaded — interactive boot was slow enough to mask it, but a fresh Playwright context with instant render hit it on the first `PrfService.HasCachedKeys()` evaluation in `PrfVfsTest.razor`'s button `Disabled` bindings. Fixed by adding public `NobleInterop.IsInitialized` predicate and gating both providers' sync paths on it. **R2 follow-up — session-expired E2E test ✅ DONE** (`e874563`). Widened `KeyCacheOptions.TtlMs` (int?) as a sub-minute override that takes precedence over `TtlMinutes`, threaded through `SecureKeyCache.Store` and `PrfService.StoreSeedAndDeriveKeysAsync` (both C# seed cache and JS-side bundle TTL). TestApp configured `TtlMs = 8000` — sub-minute, leaves the existing 4 PRF scenarios well within their post-auth budget (~1-2s of work after AuthAndOpen) and lets the new test wait ~9s. Scenario 5 — `SessionExpiresOnTtl_DropsKeyAndReEnablesAuth`: Register → AuthAndOpen → assert "Authenticate and open" disabled (HasCachedKeys true) → wait > TtlMs → expect "PRF session ended" alert → assert button re-enabled. **Latent bug surfaced by the new test:** `PrfVfsTest.razor` filtered `KeyExpired` on `"prf-key:" + salt` but the actual emitted id is `"prf-seed:" + salt` (matches `PrfService.PrfSeedCacheKey`); filter never matched, so `OnKeyExpired` was effectively dead code. Fixed by aligning the page filter to `"prf-seed:"` and updating the stale `IPrfService.KeyExpired` doc comment that listed the wrong slot prefixes. **`IPasskeyHintProvider` ✅ DONE** (`ad8c7d8`). New abstraction in `Crypto.Abstractions.Services` lets apps target the most-recently-registered passkey via WebAuthn `allowCredentials` and skip the picker UI; default `LocalStoragePasskeyHintProvider` is JSImport-backed and keyed by `"prf-hint:{salt}"`; hosts wanting cross-device hint replication for transferable iCloud / Google passkeys can swap a synced backend (e.g. CryptoSync table) via standard DI replacement. `PrfService.RegisterAsync` persists `result.RawId` (standard Base64 — Id is URL-safe Base64 which would silently fail in the TS `atob`-based `base64ToBytes`); new `DeriveKeysWithHintAsync` reads the hint and either targets it via `DeriveKeysAsync(credentialId)` or falls back to discoverable, with stale hints (any non-cancel failure) cleared automatically so a corrupted hint can't lock the user out. `PrfVfsTest.AuthenticateAndOpen` now routes through `DeriveKeysWithHintAsync`; new E2E scenario `HintTargetsLastRegisteredCredential_NoPickerWorkaroundNeeded` demonstrates hint-targeted ceremony — diverges from `CredentialMismatch_SurfacesWrongKey` by NOT calling `WebAuthn.removeCredential(A)`, deterministic outcome IS the proof that the hint path is exercised. **Build chore** (`4abc0fb`) — `src/Base/SqliteWasmBlazor/wwwroot/{noble-prf,sqlite-wasm-bridge,sqlite-wasm-worker}.js` and `sqlite3.wasm` are now `.gitignore`d; they're deterministically regenerated by the npm `<Exec>` chain on every `dotnet build`, so tracking them produced churn commits like `0fa6708 "refresh esbuild bundles"` with no SSOT value and risked silent drift. **R3 (composition) ✅ DONE.** R3.1 — `Synthetic_PrfSeed_DrivesEncryptedVfsRoundTrip` (`0dd2516`): drive a deterministic 32-byte seed through `ICryptoProvider.StoreKeysAsync` → X25519 keypair → pubkey-bytes K → `InstallEncryptionKeyAsync` → write 25 rows → close + clear → reinstall K → read rows → verify. Replaces WebAuthn with a known seed so the same composition `PrfVfsTest.razor` exercises in the browser is validated end-to-end without virtual-authenticator infrastructure. R3.2 — `Synthetic_PrfSeed_KeyRotationPreservesRowsUnderNewKey` (`6feddb9`): two distinct synthetic seeds → two distinct K_old / K_new; `ExportDatabaseAsync(REKEY) → DeleteDatabaseAsync → InstallEncryptionKeyAsync(K_new) → ImportDatabaseAsync` → rows survive under K_new and K_old returns `WRONG_KEY` on the rekeyed slot. Sanity-checks K_old != K_new up front so a regression collapsing the seed → pubkey derivation can't silently make the rotation look OK. Both R3 tests reuse `PrfVfsTestContext` (no static key — flows through worker registry) and clean up the on-disk DB before+after so the demo page sees a fresh OPFS on next manual run. **Phase 2 closeout — UI test bootstrap DROPPED (2026-05-01).** Stage 2 slot 10 (`Crypto.UI.Tests` + `CryptoSync.UI.Tests` xUnit projects) is dropped, not deferred. Rationale: panel inventory shows 5 of 8 RxBlazor models are trivial wrappers (single property + single command, e.g. `RegistrationModel` 4 props / 2 cmds at 62 lines, `DatabaseErrorAlertModel` 1 prop / 1 cmd at 47 lines, `SessionExpiredPopoverModel` 1 prop / 2 cmds at 49 lines, `PushPanel` is component-only with no model). The 3 non-trivial models (`AuthenticationModel` 9 props / 7 cmds — metadata-dialog flow + PRF-support gating; `ContactsModel` 3 cmds with delete-reload sort logic; `InvitationModel` dual-context seams + multi-part status messages) get exercised end-to-end through the demo (Phase 3.a) + TestApp Playwright lane (Phase 3.b). xUnit project bootstrap cost (~120 LOC each, no existing template) > test value at current panel complexity. Revisit only if a panel grows real logic. | ✅ DONE — R1 + sweep + R2 6/6 + R3 2/2; UI test slot dropped with rationale |
