...
export async function createGroupKeys(
    ...
    try {
        ...
        for (const memberPubKey of memberPublicKeys) {
            const wrappingKey = await deriveWrappingKey(adminPrivateKey, memberPubKey, groupContext);
            try {
...
export async function transferGroupAdmin(
    ...
    try {
        ...
        for (const memberPubKey of memberPublicKeys) {
            const wrappingKey = await deriveWrappingKey(newAdminPrivateKey, memberPubKey, groupContext);
            try {
                const wrappedCek = await wrapContentKey(cek, wrappingKey);
                memberKeys.push({ memberPublicKey: memberPubKey, wrappedContentKey: wrappedCek });
            } finally {
...
export async function encryptForGroup(
    ...
    // Unwrap sender's CEK
    const wrappingKey = await deriveWrappingKey(senderX25519PrivateKey, adminPublicKey, groupContext);
    let cek: Uint8Array;
    try {
    ...
    try {
        ...
        const envelopeSignature = await ed25519Sign(envelopeBytes, senderEd25519PrivateKey);
...
export async function decryptFromGroup(
    ...
    if (!await ed25519Verify(message.envelopeSignature, envelopeBytes, message.senderPublicKey)) {
        return PrfResultUtil.fail(PrfErrorCode.VerificationFailed);
    }

    // Layer 3: Unwrap CEK
    const wrappingKey = await deriveWrappingKey(recipientX25519PrivateKey, adminPublicKey, message.groupContext);
    let cek: Uint8Array;
    try {
...
async function unwrapMemberCek(
    ...
    const wrappingKey = await deriveWrappingKey(memberPrivateKey, adminPublicKey, groupContext);
    try {
