...
export async function encryptAsymmetric(
    ...
    const ephemeralPublicKey = await getX25519PublicKey(ephemeralPrivate);

    const sharedSecret = await x25519SharedSecret(ephemeralPrivate, recipientPublicKey);
    const encryptionKeyBytes = hkdf(sha256, sharedSecret, undefined, ECIES_INFO, 32) as Uint8Array;

    const cryptoKey = await crypto.subtle.importKey(
        'raw', toBuffer(encryptionKeyBytes), { name: 'AES-GCM' }, false, ['encrypt']
    );

    const nonce = generateRandomBytes(NONCE_LENGTH_AES);
    const ciphertext = await crypto.subtle.encrypt(
        { name: 'AES-GCM', iv: toBuffer(nonce) }, cryptoKey, toBuffer(plaintext)
    );
...
export async function decryptAsymmetric(
    ...
    const sharedSecret = await x25519SharedSecret(privateKey, encrypted.ephemeralPublicKey);
    const encryptionKeyBytes = hkdf(sha256, sharedSecret, undefined, ECIES_INFO, 32) as Uint8Array;

    const cryptoKey = await crypto.subtle.importKey(
        'raw', toBuffer(encryptionKeyBytes), { name: 'AES-GCM' }, false, ['decrypt']
    );

    const plaintext = await crypto.subtle.decrypt(
        { name: 'AES-GCM', iv: toBuffer(encrypted.nonce) }, cryptoKey, toBuffer(encrypted.ciphertext)
    );
