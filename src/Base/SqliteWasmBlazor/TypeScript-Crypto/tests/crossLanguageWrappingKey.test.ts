...
describe('cross-language wrapping-key vectors', () => {
    for (const v of VECTORS) {
        it(`deriveWrappingKey matches BouncyCastle vector — ${v.label}`, () => {
            const owner = deriveDualKeyPair(v.ownerSeed);
            const recipient = deriveDualKeyPair(v.recipientSeed);

            const actual = deriveWrappingKey(
    ...
    it('deriveWrappingKey is deterministic for the same context', () => {
        const owner = deriveDualKeyPair(ZEROS);
        const recipient = deriveDualKeyPair(ONES);

        const first = deriveWrappingKey(owner.x25519PrivateKey, recipient.x25519PublicKey, 'group:v1');
        const second = deriveWrappingKey(owner.x25519PrivateKey, recipient.x25519PublicKey, 'group:v1');

        expect(bytesToBase64(first)).toBe(bytesToBase64(second));
    });

    it('different context yields different wrapping key', () => {
        const owner = deriveDualKeyPair(ZEROS);
        const recipient = deriveDualKeyPair(ONES);

        const ctxA = deriveWrappingKey(owner.x25519PrivateKey, recipient.x25519PublicKey, 'ctx-a');
        const ctxB = deriveWrappingKey(owner.x25519PrivateKey, recipient.x25519PublicKey, 'ctx-b');
