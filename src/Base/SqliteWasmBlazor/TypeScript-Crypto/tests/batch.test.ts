...
describe('batch signatures', () => {
    ...
    it('signBatch/verifyBatch round-trip', async () => {
        ...
        const sig = await signBatch(ciphertexts, nonces, kp.privateKey);
        expect(sig.length).toBe(64);

        expect(await verifyBatch(ciphertexts, nonces, sig, kp.publicKey)).toBe(true);
    });
    ...
    it('verifyBatch rejects tampered ciphertext', async () => {
        ...
        expect(await verifyBatch(tampered, nonces, sig, kp.publicKey)).toBe(false);
    });
    ...
    it('verifyBatch rejects tampered nonce', async () => {
        ...
        expect(await verifyBatch(ciphertexts, tampered, sig, kp.publicKey)).toBe(false);
    });
    ...
    it('verifyBatch rejects wrong sender key', async () => {
        ...
        const sig = await signBatch(ciphertexts, nonces, sender.privateKey);

        expect(await verifyBatch(ciphertexts, nonces, sig, attacker.publicKey)).toBe(false);
    });
    ...
    it('verifyBatch rejects added row', async () => {
        ...
        expect(await verifyBatch(extraCt, extraN, sig, kp.publicKey)).toBe(false);
    });
    ...
    it('verifyBatch rejects removed row', async () => {
        ...
        // Remove last row
        expect(await verifyBatch(ciphertexts.slice(0, 2), nonces.slice(0, 2), sig, kp.publicKey)).toBe(false);
    });

    it('works with empty batch', async () => {
        const kp = await generateEd25519KeyPair();
        const sig = await signBatch([], [], kp.privateKey);
        expect(sig.length).toBe(64);
        expect(await verifyBatch([], [], sig, kp.publicKey)).toBe(true);
    });
