...
public class CryptoSyncBootstrap(IGroupEncryption groupEncryption, DeclarationSigner signer)
{
        ...
        string deviceName = "Admin Device")
    {
        ...
        // Priv-key bytes alias adminKeys (a DualKeyPairFull); caller owns
        // the record lifecycle and Clear()s at EOL (P21).
        var adminPrivateKeyBytes = adminKeys.X25519PrivateKey;

        {
            ...
            // Sign ShareTarget credentials with admin's Ed25519 key (alias).
            byte[] systemTargetSig;
            byte[] selfTargetSig;
            {
                systemTargetSig = await signer.SignShareTargetAsync(
                    adminKeys.Ed25519PrivateKey, adminKeys.X25519PublicKey, SyncRole.OWNER,
