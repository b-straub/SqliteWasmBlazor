...
/// </para>
/// </summary>
internal class GroupTransferService(
    CryptoSyncContextBase context,
    IGroupEncryption groupEncryption,
    DeclarationSigner signer) : IGroupTransferService
{
    /// <summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Priv-key bytes alias oldAdminKeys; caller owns DualKeyPairFull lifecycle (P21).

        // Sign the transfer declaration.
        var transferSig = await signer.SignTransferDeclarationAsync(
            oldAdminKeys.Ed25519PrivateKey, groupContext, newAdminEd25519PublicKey);
        ...
        var addMembersResult = await groupEncryption.AddGroupMembersAsync(
            oldAdminKeys.X25519PrivateKey, oldAdminKeys.X25519PublicKey,
            oldAdminWrappedCek, [newAdminX25519PublicKey], groupContext);
        if (!addMembersResult.Success)
        {
            throw new InvalidOperationException(
                $"GroupTransferService: AddGroupMembersAsync failed: {addMembersResult.ErrorCode}");
        }
        var wrappedForNewAdmin = addMembersResult.Value
            ?? throw new InvalidOperationException(
        ...
        var credSig = await signer.SignShareTargetAsync(
            oldAdminKeys.Ed25519PrivateKey, newAdminX25519PublicKey, SyncRole.OWNER,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Priv-key bytes alias newAdminKeys; caller owns DualKeyPairFull lifecycle (P21).
        var rotateResult = await groupEncryption.RotateGroupKeyAsync(
            newAdminKeys.X25519PrivateKey, newAdminKeys.X25519PublicKey,
            memberKeys, newGroupContext);
        if (!rotateResult.Success)
        {
            throw new InvalidOperationException(
                $"GroupTransferService: RotateGroupKeyAsync failed: {rotateResult.ErrorCode}");
        }
        var rotatedBundle = rotateResult.Value
            ?? throw new InvalidOperationException(
        ...
        {
            var credSig = await signer.SignShareTargetAsync(
                newAdminKeys.Ed25519PrivateKey, memberWrapped.MemberPublicKey, SyncRole.OWNER,
