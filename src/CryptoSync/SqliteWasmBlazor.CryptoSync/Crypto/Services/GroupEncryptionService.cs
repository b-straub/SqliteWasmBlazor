...
/// Provider-agnostic: works with any ICryptoProvider implementation (SubtleCryptoProvider, BouncyCastleCryptoProvider).
/// </summary>
internal sealed class GroupEncryptionService(ICryptoProvider cryptoProvider) : IGroupEncryption
{
    // ============================================================
    ...
    private static void ClearMemory(ReadOnlyMemory<byte> memory)
    {
        ...
        {
            System.Security.Cryptography.CryptographicOperations.ZeroMemory(segment.AsSpan());
        }
    }
}
