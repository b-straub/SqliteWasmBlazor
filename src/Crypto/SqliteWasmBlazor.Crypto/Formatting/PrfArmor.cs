            ...
            if (json.TrimStart().StartsWith('{'))
            {
                ...
                if (payload?.Key is not null)
                {
                    var metadata = (payload.Name is not null || payload.Email is not null ||
                                   ...
                                   payload.CredentialId is not null)
...
public sealed class PublicKeyMetadata
{
    ...
    /// <summary>
    /// WebAuthn credentialId (Base64) associated with the passkey that derived
    /// this PRF key. Carried alongside the public key so a recipient-share
    /// envelope can drive WebAuthn's <c>allowCredentials</c> to the exact
    /// passkey the sender targeted, eliminating the "which passkey?" prompt
    /// during guided imports.
    /// </summary>
    public string? CredentialId { get; init; }
}
