...
describe('key-registry (global key)', () => {
    it('setGlobalKey installs the disk key (path-agnostic)', () => {
        const key = makeKey(42);
        setGlobalKey(key);
        expect(hasGlobalKey()).toBe(true);
        // Disk-as-unit model: there is one key for the whole disk; no per-
        // path lookup. xRead / xWrite consume getGlobalKey() directly.
        expect(getGlobalKey()).toBe(key);
        clearGlobalKey();
    });

    it('clearGlobalKey wipes the buffer in place', () => {
        const key = makeKey(7);
        ...
        expect(getGlobalKey()).toBeUndefined();
    });

    it('setGlobalKey replaces the previous key, wiping it in place', () => {
        const oldKey = makeKey(11);
        const newKey = makeKey(17);
        setGlobalKey(oldKey);
        setGlobalKey(newKey);
        // The old buffer gets zeroed before being replaced.
        expect(Array.from(oldKey)).toEqual(new Array(32).fill(0));
        // The new key is live.
        expect(getGlobalKey()).toBe(newKey);
        clearGlobalKey();
    });

    it('clearing when no global key is set is a no-op', () => {
        expect(() => clearGlobalKey()).not.toThrow();
        expect(hasGlobalKey()).toBe(false);
    });

    it('rejects a non-32-byte key and wipes the bad buffer', () => {
        const bad = new Uint8Array(31);
        for (let i = 0; i < 31; i++) bad[i] = (i + 1) & 0xff;
        expect(() => setGlobalKey(bad)).toThrow(/32 bytes/);
        // The rejected buffer is wiped before the throw.
