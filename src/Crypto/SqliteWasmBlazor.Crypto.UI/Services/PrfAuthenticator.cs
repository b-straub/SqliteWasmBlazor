...
/// </para>
/// </summary>
public sealed class PrfAuthenticator : IPrfAuthenticator
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var byHintAware = await _prf.DeriveKeysWithHintAsync();
        if (byHintAware.Cancelled)
        {
            return null;
        }
        if (!byHintAware.Success)
        {
            throw new PrfAuthenticatorException(
                PrfAuthenticatorOperation.Authenticate,
                byHintAware.ErrorCode ?? PrfErrorCode.KEY_DERIVATION_FAILED);
        }
        ...
        var (credentialId, publicKey) = byHintAware.Value;
